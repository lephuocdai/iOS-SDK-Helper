...
- (void)viewDidLoad
{
	...
	[Weemo WeemoWithAppID:MOBILEAPPID
...
- (IBAction)generateAndSend:(id)sender
{
	...
	[self performSelectorInBackground:@selector(prepareMail) withObject:nil];
}
...
- (void)prepareMail
{
	...
	[composer setSubject:@"Weemo iOS SDK Helper"];
	[composer setToRecipients:nil];
	
	[composer addAttachmentData: [NSData dataWithContentsOfFile: [[Weemo instance] createLogfile]]
					   mimeType:@"text/plain" fileName:@"Weemo_iOS_SDK.log" ];
...
{
//	[self addChildViewController:composer];
	
//	[[composer view]setFrame:CGRectMake(0., 0., self.view.bounds.size.height, self.view.bounds.size.width)];
...
{
	if (![[alertView title] isEqualToString:@"Mail"])
	{
		if (buttonIndex == 0)
		{
			//user took the call
